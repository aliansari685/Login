    ...
    abstract class Operation //CRUD Operation For DB
    {
        ...
        public async Task<bool> InsertToDB(List<string> values)
        {
            try
            {
                using (context = new LoginDBEntities())
                {
                    ...
                    var birth = int.Parse(values[6].Substring(0, 4)) <= int.Parse(Shamsi_Date().Substring(0, 4)) - 15 ? values[6] : null;
                    _user = new User()
                    {
                        Row = row,
                        Firstname = values[0],
        ...
        public bool UpdateTBL(int id, List<string> values)
        {
            try
            {
                using (context = new LoginDBEntities())
                {
                    string birth = int.Parse(values[5].Substring(0, 4)) <= int.Parse(Shamsi_Date().Substring(0, 4)) - 15 ? values[5] : null;
            ...
            {
                Print(Error.Cancel, ex.InnerException.Message);
                Print(Error.DataBase, null);
                return false;
            }
        }
